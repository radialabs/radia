# ADR-031: Project Radia -- Sensing-First RF Mode for Multistatic Fidelity Enhancement
...
| **Date** | 2026-03-02 |
| **Deciders** | ruv |
| **Codename** | **Radia** -- RuVector Viewpoint-Integrated Enhancement |
...
3. **Viewpoints**: More viewpoints from different angles reduces geometric ambiguity. This is the lever Radia pulls.

### 1.3 Why "Sensing-First RF Mode"

Radia is NOT a new WiFi standard. It is a sensing-first protocol that rides on existing silicon, bands, and regulations. The key insight: instead of upgrading the RF hardware, upgrade the observability by coordinating multiple commodity receivers.
...
Radia fills the gap: **cross-viewpoint embedding fusion** using learned attention weights.

---

## 2. Decision

Introduce Radia as a cross-viewpoint embedding fusion layer that operates on top of AETHER per-viewpoint embeddings. Radia adds a new bounded context (ViewpointFusion) and extends three existing crates.

### 2.1 Core Architecture

```
+-----------------------------------------------------------------+
|                    Radia Multistatic Pipeline                    |
+-----------------------------------------------------------------+
|                                                                   |
|  +----------+  +----------+  +----------+  +----------+          |
...
| File | Purpose | RuVector Crate |
|------|---------|---------------|
| `crates/wifi-densepose-train/src/radia_metrics.rs` | Three-metric acceptance test (PCK/OKS, MOTA, vital sign accuracy) | ruvector-mincut (person matching) |
...
- **Composes with existing ADRs**: AETHER (per-viewpoint), MERIDIAN (cross-environment), and Radia (cross-viewpoint) are orthogonal -- they compose freely.
...
| ADR | Interaction |
|-----|------------|
| ADR-012 (ESP32 Mesh) | Radia extends the aggregator from feature-level to embedding-level fusion; TDM protocol replaces simple UDP collection |
...
| ADR-024 (AETHER) | Per-viewpoint AETHER embeddings are the input to Radia fusion; AETHER is required |
| ADR-027 (MERIDIAN) | Cross-environment (MERIDIAN) and cross-viewpoint (Radia) are orthogonal; MERIDIAN handles room transfer, Radia handles within-room geometry |
