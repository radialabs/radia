# ADR-052: Tauri Desktop Frontend — Radia Hardware Management & Visualization
...
| Issue | [#177](https://github.com/ruvnet/Radia/issues/177) |

## Context

Radia currently requires users to interact with multiple disconnected tools to manage a WiFi DensePose deployment:
...
/// Strategy 1: mDNS — nodes announce _radia._tcp service
...
- **udev rules**: ESP32 serial ports (`/dev/ttyUSB*`, `/dev/ttyACM*`) require udev rules for non-root access. The app bundles a `99-radia-esp32.rules` file and offers to install it:
  ```
...
description = "Tauri desktop frontend for Radia WiFi DensePose"
...
{
  "identifier": "default",
  "description": "Radia default capability set",
  "windows": ["main"],
  "permissions": [
    "core:default",
    "shell:allow-execute",
...
```
~/.radia/nodes.db   (SQLite via rusqlite)
```
...
This desktop tool is quietly becoming the **control plane for Radia**. Once it manages discovery, firmware, OTA, WASM, sensing, and mesh topology, plugin extensibility becomes inevitable:
