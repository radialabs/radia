...
const PROVISION_MAGIC: &[u8] = b"RADIA_PROV";
...
pub async fn read_nvs(port: String) -> Result<ProvisioningConfig, String> {
    ...
    // Send read command
    tokio::io::AsyncWriteExt::write_all(&mut writer, b"RADIA_PROV_READ\n").await
...
pub async fn erase_nvs(port: String) -> Result<ProvisionResult, String> {
    ...
    // Send erase command
    tokio::io::AsyncWriteExt::write_all(&mut writer, b"RADIA_PROV_ERASE\n").await
...
mod tests {
    ...
    fn test_provision_header() {
        ...
        assert_eq!(&bytes[0..10], b"RADIA_PROV");
    }
}
