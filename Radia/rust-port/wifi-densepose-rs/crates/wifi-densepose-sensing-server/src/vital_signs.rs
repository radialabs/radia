...
impl VitalSignDetector {
    ...
    pub fn compute_fft_peak(
        ...
        // Parabolic interpolation on log-magnitudes (McAulay–Quatieri-style sub-bin peak)
        let peak_freq = if peak_bin > min_bin && peak_bin < max_bin {
            let eps = 1e-15;
            let alpha = (spectrum[peak_bin - 1] + eps).ln();
            let beta = (spectrum[peak_bin] + eps).ln();
            let gamma = (spectrum[peak_bin + 1] + eps).ln();
            let denom = alpha - 2.0 * beta + gamma;
            if denom.abs() > f64::EPSILON {
                let delta = 0.5 * (alpha - gamma) / denom;
                (peak_bin as f64 + delta) * freq_res
            } else {
