//! Radia three-metric acceptance test (ADR-031).
//!
...
impl std::fmt::Display for RadiaTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RadiaTier::Fail => write!(f, "FAIL"),
            RadiaTier::Bronze => write!(f, "BRONZE"),
            RadiaTier::Silver => write!(f, "SILVER"),
            RadiaTier::Gold => write!(f, "GOLD"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct RadiaAcceptanceResult {
...
impl RadiaAcceptanceResult {
    /// A human-readable summary of the acceptance test.
    pub fn summary(&self) -> String {
        format!(
            "Radia Tier={} | PCK={:.3} OKS={:.3} | MOTA={:.3} IDsw={} | Breathing={:.1}BPM err",
            self.tier,
...
mod tests {
    ...
    fn tier_determination_gold() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), RadiaTier::Gold);
    }
    ...
    fn tier_determination_silver() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), RadiaTier::Silver);
    }
    ...
    fn tier_determination_bronze() {
        ...
        let tr = TrackingResult { passes: true, ..Default::default() };
        let vs = VitalSignResult { passes: false, ..Default::default() };
        assert_eq!(determine_tier(&je, &tr, &vs), RadiaTier::Bronze);
    }
    ...
    fn tier_determination_fail() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), RadiaTier::Fail);
    }

    #[test]
    fn tier_ordering() {
        assert!(RadiaTier::Gold > RadiaTier::Silver);
        assert!(RadiaTier::Silver > RadiaTier::Bronze);
        assert!(RadiaTier::Bronze > RadiaTier::Fail);
    }
