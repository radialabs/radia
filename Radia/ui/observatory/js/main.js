/**
 * Radia Observatory — Main Scene Orchestrator
 *
...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('radia-settings-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('radia-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('radia-observatory-settings');
        localStorage.setItem('radia-settings-version', SETTINGS_VERSION);
      }
    } catch {}
