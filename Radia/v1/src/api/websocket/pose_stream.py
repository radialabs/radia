...
class PoseStreamHandler:
    ...
    async def _get_current_pose_data(self) -> Dict[str, Any]:
        """Format current pose data by zone for WebSocket streaming."""
        try:
            result = await self.pose_service.estimate_poses()
            zone_data = {}
            for person in result.get("persons", []):
                ...
                cur = zone_data[zone_id]["confidence"]
                zone_data[zone_id]["confidence"] = (cur + person.get("confidence", 0.0)) / 2
                if not zone_data[zone_id]["activity"] and person.get("activity"):
                    zone_data[zone_id]["activity"] = person["activity"]
            return zone_data
        except Exception as e:
            logger.error(f"Error getting current pose data: {e}")
            return {}
    ...
    async def send_historical_data(self, client_id: str, zone_id: str, start_time: datetime, end_time: datetime, limit: int = 100):
        """Historical data is not available without a persistence backend."""
        await self.connection_manager.send_to_client(client_id, {
            "type": "historical_data_complete",
            "zone_id": zone_id,
            "total_records": 0,
            "note": "No persistence backend configured.",
            "timestamp": datetime.utcnow().isoformat(),
        })
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pose_status = await self.pose_service.get_status()
            return {
                "streaming": {
                ...
                "pose_service": pose_status.get("statistics", {}),
                "timestamp": datetime.utcnow().isoformat(),
            }
        except Exception as e:
