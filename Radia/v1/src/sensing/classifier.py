...
class PresenceClassifier:
    ...
    def _compute_confidence(
        ...
        # -- cross-receiver agreement (0..1): confidence-weighted match rate
        agreement = 1.0
        if other_results:
            weighted_agree = sum(
                r.confidence for r in other_results if r.motion_level == level
            )
            weighted_total = sum(r.confidence for r in other_results) + 1e-12
            agreement = weighted_agree / weighted_total
